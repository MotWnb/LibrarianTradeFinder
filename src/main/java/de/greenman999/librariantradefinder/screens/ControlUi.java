...
public class ControlUi extends Screen {
    ...
    protected void init() {
        ...
        this.addRenderableWidget(
                GrayButtonWidget.builder(Component.translatable("tradefinderui.buttons.save"), (buttonWidget) -> {
                            enchantmentsListWidget.saveToConfig();
        ...
        this.addRenderableWidget(
                GrayButtonWidget.builder(
                                Component.translatable("tradefinderui.buttons.start").withStyle(ChatFormatting.GREEN),
                                (buttonWidget) -> {
        ...
        enchantmentsListWidget = new EnchantmentsListWidget(
                this.minecraft,
                this.width / 2 - 10,
                this.height - 30,
                25,
        ...
        this.addRenderableWidget(
                GrayButtonWidget.builder(
                                getButtonText("tradefinderui.options.tp-to-villager", LibrarianTradeFinder.getConfig().tpToVillager),
                                (buttonWidget) -> {
        ...
        this.addRenderableWidget(
                GrayButtonWidget.builder(
                                getButtonText("tradefinderui.options.prevent-axe-break", LibrarianTradeFinder.getConfig().preventAxeBreaking),
                                (buttonWidget) -> {
        ...
        this.addRenderableWidget(
                GrayButtonWidget.builder(
                                getButtonText("tradefinderui.options.legit-mode", LibrarianTradeFinder.getConfig().legitMode),
                                (buttonWidget) -> {
        ...
        this.addRenderableWidget(
                GrayButtonWidget.builder(
                                getButtonText("tradefinderui.options.slow-mode", LibrarianTradeFinder.getConfig().slowMode),
                                (buttonWidget) -> {
                                    LibrarianTradeFinder.getConfig().slowMode = !LibrarianTradeFinder.getConfig().slowMode;
                                    LibrarianTradeFinder.getConfig().legitMode =
                                            LibrarianTradeFinder.getConfig().slowMode || LibrarianTradeFinder.getConfig().legitMode;
                                    updateButtonTexts();
                                })
    ...
    public boolean mouseClicked(@NonNull MouseButtonEvent click, boolean doubled) {
        for (EnchantmentEntry entry : enchantmentsListWidget.children()) {
            boolean maxPriceFieldSuccess = entry.maxPriceField.mouseClicked(click, doubled);
            boolean levelFieldSuccess = entry.levelField.mouseClicked(click, doubled);
            entry.maxPriceField.setFocused(maxPriceFieldSuccess);
            entry.levelField.setFocused(levelFieldSuccess);
        }
